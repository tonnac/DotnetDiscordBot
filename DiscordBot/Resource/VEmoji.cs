namespace DiscordBot.Resource;

public class VEmoji
{
    public static readonly string Mosquito = "\uD83E\uDD9F"; // 🦟
    public static readonly string Bat = "\uD83E\uDD87"; // 🦇
    public static readonly string Octopus = "\uD83D\uDC19"; // 🐙
    public static readonly string Shark = "\uD83E\uDD88"; // 🦈
    public static readonly string Unicorn = "\uD83E\uDD84"; // 🦄
    public static readonly string Skeleton = "\uD83D\uDC80"; // 💀
    public static readonly string Devil = "\uD83D\uDE08"; // 😈
    public static readonly string SlotMachine = "\uD83C\uDFB0"; // 🎰
    public static readonly string Alien = "\uD83D\uDC7D"; // 👽
    public static readonly string AngryDevil = "\uD83D\uDC7F"; // 👿
    public static readonly string Trex = "\uD83E\uDD96"; // 🦖
    public static readonly string Crab = "\uD83E\uDD80"; // 🦀
    public static readonly string Dragon = "\uD83D\uDC09"; // 🐉
    public static readonly string TheOffice = "\uD83C\uDFEC"; // 🏬

    public static readonly string Heart = "\u2665\uFE0F"; // ♥️
    public static readonly string ThumbsUp = "\uD83D\uDC4D"; // 👍
    public static readonly string GiftBox = "\uD83C\uDF81"; // 🎁
    public static readonly string CardFileBox = "\uD83D\uDDC3\uFE0F"; // 🗃️
    public static readonly string Dice = "\uD83C\uDFB2"; // 🎲
    public static readonly string Robot = "\uD83E\uDD16"; // 🤖
    
    public static readonly string WingMoney = "\uD83D\uDCB8"; // 💸
    public static readonly string Money = "\uD83D\uDCB0"; // 💰
    public static readonly string ConfettiBall = "\uD83C\uDF8A"; // 🎊
    
    public static readonly string LockedWithKey = "\uD83D\uDD10"; // 🔐

    public static readonly string Trophy = "\uD83C\uDFC6"; // 🏆
    public static readonly string GoldMedal = "\uD83E\uDD47"; // 🥇
    public static readonly string SilverMedal = "\uD83E\uDD48"; // 🥈
    public static readonly string BronzeMedal = "\uD83E\uDD49"; // 🥉

    public static readonly string Boom = "\uD83D\uDCA5"; // 💥
    public static readonly string Fire = "\uD83D\uDD25"; // 🔥
    public static readonly string SpiralEyes = "\ud83d\ude35\u200d\ud83d\udcab"; // 😵‍💫
    public static readonly string CryingFace = "\uD83D\uDE2D"; // 😭

    public static readonly string HammerAndPick = "\u2692\uFE0F"; // ⚒️
    public static readonly string Ring = "\uD83D\uDC8D"; // 💍
    public static readonly string Weapon = "\uD83D\uDDE1\uFE0F"; // 🗡️
    
    public static readonly string GreenCircle = "\uD83D\uDFE2"; // 🟢
    public static readonly string RedCircle = "\uD83D\uDD34"; // 🔴
    
    public static readonly string GreenCheckBox = "\u2705"; // ✅
    public static readonly string RedCrossMark = "\u274C"; // ❌
    public static readonly string QuestionMark = "\u2753"; // ❓

    public static readonly string Magnifier = "\uD83D\uDD0D"; // 🔍
    public static readonly string CrossSword = "\u2694\uFE0F"; // ⚔️
    public static readonly string Crossbones = "\u2620\uFE0F"; // ☠️

    public static readonly string FishingPole = "\uD83C\uDFA3"; // 🎣
    public static readonly string Hook = "\uD83E\uDE9D"; // 🪝
    public static readonly string Shoe = "\uD83D\uDC5F"; // 👟
    public static readonly string Fish = "\uD83D\uDC1F"; // 🐟
    public static readonly string Blowfish = "\uD83D\uDC21"; // 🐡
    public static readonly string TropicalFish = "\uD83D\uDC20"; // 🐠
    public static readonly string Merperson = "\uD83E\uDDDC"; // 🧜 
}